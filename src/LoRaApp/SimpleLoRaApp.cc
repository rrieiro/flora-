...
void SimpleLoRaApp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        //lastMessageTime = simTime()+timeToFirstPacket;
        ...
        messageDelayVector.setName("messageDelay");
        elapsedTimeVector.setName("elapsedTime");
        MCEnergyVector.setName("MCEnergyVector");
        sensorEnergyVector.setName("sensorEnergyVector");
...
void SimpleLoRaApp::finish()
{
    ...
    recordScalar("totalMCEnergyConsumed", totalMCEnergyConsumed);
    recordScalar("totalSensorEnergyConsumed", totalSensorEnergyConsumed);
}


void SimpleLoRaApp::receiveSignal(cComponent *source, simsignal_t signal, long value, cObject *details) {
    EV << "receiveSignal :" << signal << ", value = " << value << "\n";
    if (value == inet::physicallayer::IRadio::RADIO_MODE_OFF ||
        value == inet::physicallayer::IRadio::RADIO_MODE_SLEEP) {
        LoraNicEndTime = simTime();
    } else {
        if (NictimeConsumed) {
            LoraNicStartTime = simTime();
            NictimeConsumed = false;
        }
        ...
        if (msg == sendMeasurements)
        {
            ...
            if(numberOfPacketsToSend == 0 || sentPackets < numberOfPacketsToSend)
            {
                ...
                */
                ...
                simtime_t elapsedTime = now - lastTime;
                lastTime = now;
                elapsedTimeVector.record(elapsedTime.dbl());

                simtime_t messageDelayTime = LoraNicEndTime - LoraNicStartTime;
                messageDelayVector.record(messageDelayTime.dbl());
                NictimeConsumed = true;
...
void SimpleLoRaApp::handleMessageFromLowerLayer(cMessage *msg)
{
    ...
    EV << "Message Type is :" << packet->getMsgType() << endl;
